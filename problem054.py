...
def rank(hand):
	...
	hand2 = hand.split(' ')
	#print hand2
	numbers = [card_val(i[0]) for i in hand2]
	suits = [i[1] for i in hand2]
	sorted_numbers = sorted(numbers)
	if sorted_numbers == [10, 11, 12, 13, 14] and len(set(suits)) == 1:
		print ''' royal flush '''
		return 10

	if len(set(numbers)) == 5 and sorted_numbers[-1] - sorted_numbers[0] == 4 and len(set(suits)) == 1:
		print ''' Straigt Flush '''
		return 9 + sorted_numbers[-1] / 100.

	if len(set(numbers)) == 2 and sorted_numbers.count(sorted_numbers[0]) in [1, 4]:
		print ''' Four of a kind '''
		to_add = sorted_numbers[0] / 100.
		if sorted_numbers.count(sorted_numbers[0]) == 1:
			to_add = sorted_numbers[0] / 100.
		return 8 + to_add

	if len(set(numbers)) == 2 and sorted_numbers.count(sorted_numbers[0]) in [3, 2]:
		#print ''' Full House ''', sorted_numbers
		to_add = sorted_numbers[0] / 100. + sorted_numbers[-1] / 1000.
		if sorted_numbers.count(sorted_numbers[0]) == 2:
			to_add = sorted_numbers[0] / 1000. + sorted_numbers[-1] / 100.
		#print to_add
		return 7 + to_add
	...
	if len(set(numbers)) == 5 and sorted_numbers[-1] - sorted_numbers[0] == 4:
		#print ''' Straigt ''', sorted_numbers
		to_add = sorted_numbers[-1] / 100.
		return 5 + to_add

	if numbers.count(sorted_numbers[2]) == 3:
		#print ''' three of a kind ''', sorted_numbers
		to_add = sorted_numbers[2] / 100.
		not_of_3 = sorted(filter(lambda x: sorted_numbers.count(x) != 3, sorted_numbers))
		to_add2 = not_of_3[0] / 10000. + not_of_3[-1] / 1000.
		return 4 + to_add + to_add2

	if sorted([numbers.count(x) for x in sorted_numbers]) == [1, 2, 2, 2, 2]:
		#print ''' two pairs ''', sorted_numbers
		pairs = sorted(filter(lambda x: sorted_numbers.count(x) == 2, sorted_numbers))
		not_pair = filter(lambda x: sorted_numbers.count(x) != 2, sorted_numbers)
		to_add = pairs[0] / 1000. + pairs[-1] / 100. + not_pair[0] / 10000.

		return 3 + to_add

	if 2 in (numbers.count(x) for x in numbers):
		pairs = sorted(filter(lambda x: sorted_numbers.count(x) == 2, sorted_numbers))
		not_pair = filter(lambda x: sorted_numbers.count(x) != 2, sorted_numbers)
		to_add = pairs[0] / 100. + sum(not_pair) / 10000.
		return 2 + to_add
...
dealings = """8C TS KC 9H 4S 7D 2S 5D 3S AC
...
AS KD 3D JD 8H 7C 8C 5C QD 6C""".splitlines()

#print dealings[0][:3*5-1], 
print "yo", sum([rank(d[:3*5-1]) > rank(d[3*5:]) for d in dealings])
#	player1 = d[:3*5]
#   player2 = d[3*5:]



print rank("5H 5C 6S 7S KD") < rank("2C 3S 8S 8D TD")
print rank("5D 8C 9S JS AC") > rank("2C 5C 7D 8S QH")
print rank("2D 9C AS AH AC") < rank("3D 6D 7D TD QD")
print rank("4D 6S 9H QH QC") > rank("3D 6D 7H QD QS")
print rank("2H 2D 4C 4D 4S") > rank("3C 3D 3S 9S 9D")